...
import { ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Sparkles, X, MessageCircle, CheckCircle2, Circle, Flower2, Clock, Zap, Eye, Ear, Fingerprint, Wind, Apple, Info } from 'lucide-react';
...
const ArchiveView: React.FC<Props> = ({ data, onUpdateData }) => {
  // Calendar State
  const [currentDate, setCurrentDate] = useState(() => {
      // Initialize based on today's system date
  ...
  const lunarDetails = useMemo(() => {
      ...
      return getLunarDetails(dateObj);
  }, [selectedDateStr]);
  
  const displayMoonPhase = entryForSelection ? entryForSelection.moonPhase : lunarDetails.phase;
  ...
  const sensoryLogs = selectedEntry.sensoryLogs || [];

  // --- GOAL TOGGLE LOGIC ---
  const toggleGoalCompletion = () => {
      if (!entryForSelection || !entryForSelection.tomorrowSeed) return;
      
      const newStatus = !entryForSelection.tomorrowSeed.isCompleted;
      
      const updatedEntry: DayEntry = {
          ...entryForSelection,
          tomorrowSeed: {
              ...entryForSelection.tomorrowSeed,
         ...
         <div className="mt-4 text-center w-full max-w-sm animate-fade-in-up flex flex-col items-center">
             ...
             >
                 <div className="bg-slate-800/60 backdrop-blur-xl border border-white/10 p-5 rounded-2xl shadow-2xl mx-auto space-y-4">
                     ...
                     {card ? (
                         ...
                         >
                             ...
                             <div className="text-left flex-1">
                                 {/* Display selectedTitle as main name if available, otherwise card name */}
                                 <h4 className="text-white text-sm font-medium">
                                     {selectedEntry.todayAwareness?.selectedTitle || card.name}
                                 </h4>
                                 {/* Display card name as subtitle if title exists */}
                                 <p className="text-indigo-200 text-xs mt-1 line-clamp-1 italic">
                                     {selectedEntry.todayAwareness?.selectedTitle ? card.name : '未命名'}
                                 </p>
                     ...
                     {/* Goal Status - INTERACTIVE */}
                     {seedGoal && seedGoal.energySeed && (
                         <div 
                             className="bg-white/5 p-3 rounded-xl border border-white/5 flex items-start space-x-3 cursor-pointer hover:bg-white/10 transition-all group"
                             onClick={toggleGoalCompletion}
                         >
                             <div className={`mt-0.5 transition-colors duration-300 ${seedGoal.isCompleted ? 'text-amber-400' : 'text-slate-500 group-hover:text-slate-300'}`}>
                             ...
                             <div className="text-left">
                                 ...
                                 <p className={`text-white text-sm font-serif transition-opacity ${seedGoal.isCompleted ? 'opacity-50 line-through' : 'opacity-100'}`}>"{seedGoal.energySeed}"</p>
                             </div>
                         </div>
                     )}
                     ...
                     {sensoryLogs.length > 0 && (
                         <div className="bg-slate-900/40 p-3 rounded-xl border border-white/5">
                             ...
                             <div className="space-y-2">
                                 {sensoryLogs.map((log, idx) => {
                                     const SenseIcon = getSenseIcon(log.senseId);
                                     return (
                                        <div key={idx} className="text-sm p-3 bg-white/5 rounded-lg border border-white/5 text-left">
                                            <div className="flex items-center mb-1 text-xs text-indigo-200/70">
                                                <SenseIcon size={10} className="mr-1.5" />
                                                <span className="tracking-wide">{log.senseTitle}</span>
                                            </div>
                                            <p className="text-slate-400 text-xs italic mb-2">"{log.prompt}"</p>
                                            <p className="text-white font-serif border-l-2 border-indigo-500/30 pl-2">
                                                {log.content}
                                            </p>
                                        </div>
                                     );
                     ...
                     {/* Moon Knowledge - UPDATED with Realistic Illumination */}
                     <div className="bg-indigo-900/20 p-4 rounded-xl border border-indigo-500/20 text-left">
                         <div className="flex justify-between items-center mb-2">
                            <h4 className="flex items-center text-indigo-300 text-xs font-bold uppercase tracking-wider">
                                <Sparkles size={12} className="mr-2" />
                                {moonInfo.cnName} · 能量状态
                            </h4>
                            <span className="flex items-center text-[10px] text-indigo-200/60 bg-indigo-500/10 px-2 py-0.5 rounded-full border border-indigo-500/10">
                                <Info size={10} className="mr-1" />
                                照明度 {lunarDetails.illumination}%
                            </span>
                         </div>
         ...
         <div className="flex-1 overflow-y-auto no-scrollbar">
            <div className="grid grid-cols-7 gap-y-1 auto-rows-min pb-4">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                     ...
                     const gridLunarDetails = getLunarDetails(new Date(year, month, day));
                     let displayPhase = gridLunarDetails.phase;
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-slate-900 border border-white/10 w-full max-w-md h-[80vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden relative">
                ...
                <div className="p-6 bg-gradient-to-b from-indigo-950 to-slate-900 flex flex-col items-center border-b border-white/5">
                    ...
                    {/* MODIFIED: Prioritize Selected Title */}
                    <h3 className="text-white font-serif text-xl">{selectedEntry.todayAwareness.selectedTitle || card.name}</h3>
                    <p className="text-indigo-200 text-sm mt-1">{selectedEntry.todayAwareness.selectedTitle ? card.name : ""}</p>
